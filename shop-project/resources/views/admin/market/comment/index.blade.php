            ...
            <table class="table table-striped table-hover">
                ...
                @foreach($comments as $key => $comment)
                    <tr>
                        ...
                        <td>{{ Str::limit($comment["commentable"]["name"], 10) }}</td>
                        <td>{{ $comment["approved"] == 1 ? "تایید شده" : "تایید نشده" }}</td>

                        <td>
                            <label class="switch">
                                <input id="{{ $comment["id"] }}" onchange="changeStatus({{ $comment["id"] }})" data-url="{{ route("admin.market.comment.status" , $comment["id"]) }}" type="checkbox"
                                       @if($comment['status'] === 1)
                                       checked
                                    @endif
                                >
                        ...
                        <td class="max-width-18 text-left">
                            <a href="{{ route("admin.market.comment.show", $comment["id"]) }}" class="btn btn-sm btn-info border-radius-2 mb-2 mb-md-0"><i class="fas fa-eye ml-2"></i>نمایش</a>

                            @if($comment["approved"] == 1)
                                <a href="{{ route("admin.market.comment.approved", $comment["id"]) }}" class="btn btn-sm btn-warning border-radius-2"><i class="fas fa-ban ml-2"></i>عدم تایید</a>
                            @else
                                <a href="{{ route("admin.market.comment.approved", $comment["id"]) }}" class="btn btn-sm btn-success border-radius-2"><i class="fa fa-check-circle ml-2"></i>تایید</a>
                            @endif
                        </td>
                    </tr>
                @endforeach
